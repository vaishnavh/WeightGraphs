...
def self_similarity(G, distances):
    pwl = []
    for node in tqdm(np.random.choice(G.nodes(),size=1000)):
        for d in distances:
            pwl += [(d,len(nx.single_source_dijkstra_path_length(G,node,cutoff=d,weight='weight_inv')))]
    pwl=np.asarray(pwl)
